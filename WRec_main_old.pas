...
Type
     ...
     SpRec=Record
       ...
       nam : string[150];//array[1..232] of char;
     End;
     pt= String[200];
...
//------------------------------------------------------------------------------
{$R *.dfm}
Function Cnvrt(var p:pt; lng,wd: byte): pt;
var k,n : byte;
begin
 if wd=0 then for k:=1 to lng do begin            // OEMToChar(PChar(gg),PChar(hh));
   n:=pos(p[k], sDos); if n>0 then p[k]:=sWin[n]; // CharToOEM(PChar(gg),PChar(hh));
 end else for k:=1 to lng do begin
   n:=pos(p[k], sWin); if n>0 then p[k]:=sDos[n];
 end;
 Result:=p;
end;
//--------------------------------------------------------- ASCII <> ANSI ------
Procedure Conv(p:parr; lng,wd: byte);
var k,n : byte;
begin
 if wd=0 then for k:=1 to lng do begin
   n:=pos(p^[k], sDos); if n>0 then p^[k]:=sWin[n];
 end else for k:=1 to lng do begin
   n:=pos(p^[k], sWin); if n>0 then p^[k]:=sDos[n];
 end;
end;
...
var
   len: Byte;
begin
stt:=str;    len:= length(str);
if (len>1) then while (Stt[len] = ' ') do Dec(len); SetLength(Str,len);
RTrim := Str;
end;
//------------------------------------------------------------------------------
procedure Archivate;
...
begin
  If MessageBox(main.handle, 'Будет выполнена архивация данных! Начать?', 'Архивация', MB_YESNO)=IDYES then begin
     ...
     commline := '"'+appdir+'pkzipc.exe" -add -rec -dir=current .\BAK\'+FormatDateTime('yyyy-mm-dd',now)+'_BIO.ZIP .\rec\*.*';
     if (bak_opts[1] = '1') then commline := commline + ' .\alg\*.*';
     if (bak_opts[2] = '1') then commline := commline + ' .\main\*.*';
     if (bak_opts[3] = '1') then commline := commline + ' .\pat\*.*';
     if (bak_opts[4] = '1') then commline := commline + ' .\prt\*.*';
...
// ------------------------------------------------------------------------ Show Spectr TREE --------
Procedure ShBsm(fn:string; ti:TTreeNode);
var st:string[200];
    F : text;
Begin
  AssignFile(F, AppDir+fn);
  Reset(f); ReadLn(F); ReadLn(F); ReadLn(F); ReadLn(F); ReadLn(F);
  While Eof(F)=false do begin
    inc(nl); new(Lrec[nl]); Sclear(st); ReadLn(F,st); st:=Cnvrt(st,length(st),0);
    Lrec[nl]^.ni:=Main.SP_MENU.Items.AddChild(ti,st); Lrec[nl]^.ttl:=st;
    Sclear(st); ReadLn(F,st); St:=Cnvrt(st,length(st),0);Lrec[nl]^.dsc:=st; ReadLn(F,Lrec[nl]^.fn);
    if pos('.bsm',Lrec[nl]^.fn)>0 then  ShBsm(Lrec[nl]^.fn,Lrec[nl]^.ni);
  end; CloseFile(F);
end;
//------------------------------------------------------------------------- Show Spectr List -------
procedure ShSp(fn:pt);
var fl,ff,i,j,n:integer;
       st:string[100];
       ssp,sn:string;
begin
//  fl:=FileOpen(AppDir+fn,fmOpenRead); ff:=FileOpen(AppDir+'main\spectr.bio',fmOpenRead);
  if ff>-1 then begin
    lf := (FileSeek(fl,0,2)) div 5; FileSeek(fl,0,0); main.SP_list.RowCount:=lf;
    db := TSqliteDatabase.Open(dir+'DB\BIO_SP.db3');
    sql:='SELECT * FROM T_SP WHERE nc="'+copy(fn,2,2)+'"';
    res:=TSqliteQueryResults.Create(db, sql); i:=1;
    repeat
      ...
      Srec[i]^.nam:=UTP8Decode(res.FieldAsString(4);
      st:=res.FieldAsString(2)+res.FieldAsString(3);
      main.SP_list.Rows[i-1].Add(st);
      inc(i); res.Next;
    until res.Eof; res.Free; db.Free;
  end else begin
    MessageDlg('База спектров занята другим приложением!',mtWarning,[mbOk],0);
    main.OnClose(main);
  end;
end;
//--------------------------------------------------------------------------- Show Recept -----------
...
begin
  ...
  ff:=FileOpen(AppDir+'main\spectr.bio',fmOpenRead);
  if ff>-1 then begin
    db:=TSqliteDatabase.Open(dir+'DB\BIO_REC.DB3');
    ...
    for i:=1 to rf do begin
      ...
      Fileseek(ff,n*242,0); FileRead(ff,Rec[i]^.n,4); FileRead(ff,Rec[i]^.ns,6);
      FileRead(ff,Rec[i]^.nam,150); conv(@(Rec[i]^.nam),150,0);
      st:=Rec[i]^.ns[1]+Rec[i]^.ns[2]+'-'+Rec[i]^.ns[3]+Rec[i]^.ns[4]+'-'+
          Rec[i]^.ns[5]+Rec[i]^.ns[6]+'  '+Rec[i]^.nam;
      main.grid.Rows[i-1].Add(st);
    end; FileClose(ff); Buttons(0,1,1,1,1);
    Chr:=0; Rec_nam:=pn; res.Free; db.Free;
  end else begin
    MessageDlg('База спектров занята другим приложением!',mtWarning,[mbOk],0);
    main.OnClose(main);
  end;
...
begin
  ...
  case indx of
     0: begin fn:='alg\biokas.bsm'; Bt_Find.Default := false; end;
     1: begin fn:='alg\meridian.bsm'; Bt_Find.Default := false; end;
     2: begin fn:='alg\kl.bsm'; Bt_Find.Default := false; end;
     3: begin ClrFind;  end;
  end;
...
var arcd : TDateTime;
    ...
    cprt : string;
begin
 sWin:='абвгдежзийклмнопрстуфхцчшщъыьэюяАБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ';
 //sWn:='абвгдежзийклмнопрстуфхцчшщъыьэюяабвгдежзийклмнопрстуфхцчшщъыьэюя';
 sDos:=#160#161#162#163#164#165#166#167#168#169#170#171#172#173#174#175#224+
 #225#226#227#228#229#230#231#232#233#234#235#236#237#238#239#128#129#130#131+
 #132#133#134#135#136#137#138#139#140#141#142#143#144#145#146#147#148#149#150+
 #151#152#153#154#155#156#157#158#159;
 nl:=0; ShBsm('alg\biokas.bsm',nil);  Chr:=0;
 Buttons(0,0,0,0,0);
 flMeasureNK := false;
...
begin
  WITH Sender AS TStringGrid DO begin
    if ats[wRow]<2 then Canvas.Font.Color := clBlack else Canvas.Font.Color := clSilver;
    Canvas.Brush.Color := clWindow;
    if (gdSelected IN State) and (Sender=ActiveControl) then begin
...
begin
 ...
 if (ActiveControl=SP_List) or (Sender=SP_menu) or (ActiveControl=SP_Find) then begin
    ...
    Linfo.Font.Color:=clGray; Linfo.Text:=Slist_n; Linfo.Text:= RTrim(Linfo.Text);
 ...
 if (ActiveControl=Grid) then begin
    ...
    if (grid.row<rf) then begin
        tmp:=Rec_n; Linfo.Text:=RTrim(tmp);
  ...
  if ((key='s') or (key='S') or (key='Ы') or (key='ы')or (key='і') or (key='І')) and (ActiveControl=R_List) and (R_List.Text<>'') then begin
     if (kf=0) then begin
       ...
       if((nsel=0) or ((nsel>0) and (SelRList.IndexOf(R_List.Text)=-1))) then begin
         ...
         LabSel.Font.Color:=clRed; LabSel.Caption:=Inttostr(nsel);
         ...
         if(nsel=0) then begin
            LabSel.Visible:=False; SelRList.Free; N26.Enabled:=False;
         end;
       end;  kf:=1;
     end else kf:=0;
  end;
...
begin
  if (srec[1]<>nil) and (indx<3) then begin
    x:=0; for i:=1 to rf do if rec[i]^.ns=Srec[Sp_list.row+1]^.ns then x:=1;
    if (x=0) and (Srec[Sp_list.row+1]^.sp[5]<2) then begin
  ...
  if (Frec[1]<>nil) and (indx=3) then begin
    x:=0; for i:=1 to rf do if rec[i]^.ns=Frec[Sp_Find.row+1]^.ns then x:=1;
    if (x=0) and (Frec[Sp_Find.row+1]^.sp[5]<2) then begin
...
begin
  ...
  if k>0 then begin
    ...
    if (R_n='') then begin
      ...
      sql:=sql+',"'+R_n+'","'+DateToStr(Now)+'","'+sdesc+'","'+ssp+'")';
    end else begin
      sql:='UPDATE T_REC SET md="'+DateToStr(Now)+'",desc="'+sdesc+'",sp="'+ssp+'"';
...
begin
   if R_List.Items.Count>0 then begin
      ...
      Result:=kk;
      if(nsel>0) then begin
        N26.Enabled:=false; nsel:=0; LabSel.Visible:=False; SelRList.Free;
      end;
   end else Result:=0;
end;
//-----------------------------------------------------------------------Кл. меню -Сохранить------------
...
begin
  if (Rec_nam<>'') and (MessageDlg('Удалить рецепт?',mtConfirmation,
     ...
     if ((nsel>0) and (SelRList.IndexOf(R_List.Text)<>-1)) then begin
       ...
       if (nsel=0) then begin
         N26.Enabled:=false; LabSel.Visible:=False; SelRList.Free;
       end;
     end;
...
begin
  ...
  ff:=FileOpen(AppDir+'main\spectr.bio',fmOpenRead);
  if ff>-1 then begin
    lf := (FileSeek(ff,0,2)) div 242; FileSeek(ff,0,0);
    for i:=1 to lf do begin
      FileRead(ff,rc.n,4); FileRead(ff,rc.ns,6);
      FileRead(ff,rc.nam,150);FileRead(ff,st,82); conv(@(rc.nam),150,0);
      if Pos(AnsiLowerCase(Ed_Find.Text),AnsiLowerCase(rc.nam)) <> 0 then begin
        stt:=IntToHex(i,4); for kk:=1 to 4 do char(rc.sp[kk]):=stt[kk]; rc.sp[5]:=0;
        inc(lfl); new(FRec[lfl]); FRec[lfl].sp:=rc.sp; FRec[lfl].n:=rc.n; FRec[lfl].ns:=rc.ns;
        FRec[lfl].nam:=rc.nam;
        st:=rc.ns[1]+rc.ns[2]+'-'+rc.ns[3]+rc.ns[4]+'-'+rc.ns[5]+rc.ns[6]+'  '+rc.nam;
        SP_Find.Rows[lfl-1].Text:=st; SP_Find.RowCount:=lfl;
      end;
    end;
    FileClose(ff);
  end else MessageDlg('База спектров занята другим приложением!',mtWarning,[mbOk],0);
end;
//---------------------------------------------------------------------------
...
begin
  ...
  ini.Free;
  DeleteFile(appdir + 'PRT\RPT.HTML');
end;
//-------------------------------------------------------------------- РЕЖИМ "СПЕКТР" -------
...
clrlist();
   if main.SP_Menu.SelectionCount>0 then begin
     if pos('.bsm',LRec[main.SP_Menu.Selected.AbsoluteIndex+1]^.fn)=0 then begin
        Slist_n:=LRec[main.SP_Menu.Selected.AbsoluteIndex+1]^.dsc;
        ShSp(LRec[main.SP_Menu.Selected.AbsoluteIndex+1]^.fn);
        OnSCLick(SP_Menu);
     end;
   end;
end;
...
begin
   if main.SP_Menu.SelectionCount>0 then begin
      if (key=40) and (main.SP_Menu.Selected.AbsoluteIndex<nl-1) then begin       //DOWN
        clrlist();
        if pos('.bsm',LRec[main.SP_Menu.Selected.AbsoluteIndex+2]^.fn)=0 then begin
          Slist_n:=LRec[main.SP_Menu.Selected.AbsoluteIndex+2]^.dsc;
          ShSp(LRec[main.SP_Menu.Selected.AbsoluteIndex+2]^.fn);
          OnSCLick(SP_Menu);
        end;
      end;
      if (key=38) and (main.SP_Menu.Selected.AbsoluteIndex>0) then  begin         //UP
        clrlist();
        if pos('.bsm',LRec[main.SP_Menu.Selected.AbsoluteIndex]^.fn)=0 then begin
          Slist_n:=LRec[main.SP_Menu.Selected.AbsoluteIndex]^.dsc;
          ShSp(LRec[main.SP_Menu.Selected.AbsoluteIndex]^.fn);
          OnSCLick(SP_Menu);
        end;
      end;
   end;
